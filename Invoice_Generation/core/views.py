...
from .tasks import send_invoice_email,send_admin_mail
...
class ViewAllCustomer(AdminRequiredMixin,View):
    def get(self,request):
        query = request.GET.get('q', '').strip()
        if query:
            prod = User.objects.filter(
                Q(email__icontains=query) | Q(name__icontains=query) | Q(address__icontains=query),
                role='client'
            )
        else:
        ...
        return render(request, 'ViewCustomer.html', {'emp_data': page_obj, 'query': query})
...
class HandleProducts(AdminRequiredMixin,View):
    def get(self,request):
        ...
        page_num=request.GET.get('page')
        page_obj=pagin.get_page(page_num)
        return render(request,'HandleProducts.html',{'products':page_obj, 'query': query})
